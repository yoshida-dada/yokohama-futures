...
def find_pdf() -> Path:
    ...
    if not pdfs:
        raise FileNotFoundError("PDFファイルがフォルダに見つかりません")
    if len(pdfs) > 1:
        print(f"複数のPDFが見つかりました。最初のファイルを使用します: {pdfs[0].name}")
    return pdfs[0]
...
def parse_game_header(line: str) -> dict | None:
    ...
    """
    m = re.match(
        r'(\d+)月(\d+)日([A-Z])\s+(\S+)\s+(.+?)\s+(勝|負|引き分け)\s+(\d+対\d+)',
        line
    )
    if not m:
        return None
    month, day, team, tournament, opponent, result, score = m.groups()
...
# 打席行のパース
# ────────────────────────────────────────────
def parse_pa_row(tokens: list[str]) -> tuple[int, list]:
    """
    打席行フォーマット:
...
# 試合オブジェクトの構築
# ────────────────────────────────────────────
def build_game(header: dict, stat_rows: dict) -> dict:
    pa_tokens = stat_rows.get("pa", [])
    total_pa, pa_values = parse_pa_row(pa_tokens)
...
# PDFからの抽出
# ────────────────────────────────────────────
def extract_batter_games(pdf_path: Path) -> list[dict]:
    page_texts = []
    with pdfplumber.open(pdf_path) as pdf:
        total = len(pdf.pages)
        if total < 4:
            raise ValueError(f"PDFのページ数が不足しています（{total}ページ）。P3・P4が必要です。")
        for page in pdf.pages[2:4]:  # P3, P4 (0インデックスで 2, 3)
        ...
        if any(re.match(p, line) for p in SKIP_PATTERNS):
            ...
            if "全試合累計" in line or "直近" in line:
                if current_header and stat_rows:
                    games.append(build_game(current_header, stat_rows))
                    current_header = None
                    stat_rows = {}
            continue
...
def game_id(g: dict) -> str:
    return f"{g['date']}|{g['tournament']}|{g['opponent']}"
...
def main():
    ...
    print("打者データを抽出中 (P3・P4)...")
    try:
        pdf_games = extract_batter_games(pdf_path)
    ...
    if update_mode:
        ...
        print(f"\n※ 更新するには: python convert_batter.py --update")
